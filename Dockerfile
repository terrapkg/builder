...
RUN dnf5 in -y --setopt=install_weak_deps=False --repofrompath 'terra,https://repos.fyralabs.com/terra$releasever' --setopt='terra.gpgkey=https://repos.fyralabs.com/terra41/key.asc' terra-{release{,-extras},mock-configs} subatomic-cli anda{,-srpm-macros} mock-scm adoptium-temurin-java-repository gh git-lfs wget less podman fuse-overlayfs dnf5-plugins script mold which sudo

# Hack to fix %dist
RUN sed -i 's/.fc%{fedora}/.fc41/g' /usr/lib/rpm/macros.d/macros.dist
