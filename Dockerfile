...
RUN dnf5 -y --setopt=install_weak_deps=False install terra-mock-configs subatomic-cli anda mock rpm-build mock-scm rpmlint git-core curl wget less podman fuse-overlayfs

RUN dnf5 remove -y dnf dnf-plugins-core
RUN dnf5 clean all
